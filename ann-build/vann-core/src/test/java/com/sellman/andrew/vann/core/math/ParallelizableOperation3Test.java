...
public class ParallelizableOperation3Test {

	@Mock(answer = Answers.CALLS_REAL_METHODS)
	private ParallelizableOperation3<AbstractOperationByRowTask, AbstractOperationByColumnTask> op;
	...
	public void doMatrixOperationAsSequential() throws Exception {
		m = new Matrix(2, 1);
		doReturn(matrixTarget).when(op).doSequentialOp(eq(m), eq(2), eq(1));

		Matrix result = op.doOperation(m);
		assertEquals(matrixTarget, result);
		verifyZeroInteractions(byColumnTaskPool);
		verifyZeroInteractions(byRowTaskPool);
		verifyZeroInteractions(taskService);
	...
	public void doVectorOperationAsParallel() throws Exception {
		...
		verifyZeroInteractions(byRowTaskPool);
		verify(op).populateTask(eq(columnTask), any(CountDownLatch.class), eq(v.getMatrix()), eq(null), eq(null), any(Matrix.class), eq(null), eq(0));
	...
	public void doMatrixOperationAsParallelByColumn() throws Exception {
		...
		verify(byColumnTaskPool).recycle(columnTasks);
		verifyZeroInteractions(byRowTaskPool);
		verify(op).populateTask(eq(columnTask), any(CountDownLatch.class), eq(m), eq(null), eq(null), any(Matrix.class), eq(null), eq(0));
		verify(taskService).runTasks(columnTasks);
	}
	...
	public void doMatrixOperationAsParallelByRow() throws Exception {
		m = new Matrix(1, 2);
		setOpAsParallel(true);
		doReturn(matrixTarget).when(op).getTarget(1, 2);
		...
		verifyZeroInteractions(byColumnTaskPool);
		verify(op).populateTask(eq(rowTask), any(CountDownLatch.class), eq(m), eq(null), eq(null), any(Matrix.class), eq(null), eq(0));
