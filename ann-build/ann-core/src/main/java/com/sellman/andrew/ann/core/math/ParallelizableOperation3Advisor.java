...
class ParallelizableOperation3Advisor {
	...
	public boolean doAsParrallelOp(final ParallelizableOperation3 op, final int matrixRowCount, final int matrixColumnCount) {
	...
	private double getSequentialOpNanos(final ParallelizableOperation3 op, final Matrix m) {
		long start = System.nanoTime();
		op.doSequentialOp(m);
		return (System.nanoTime() - start);
	}

	private long getParallelOpNanos(final ParallelizableOperation3 op, final Matrix m) {
		long start = System.nanoTime();
		op.doParallelOp(m);
		return (System.nanoTime() - start);
	}
