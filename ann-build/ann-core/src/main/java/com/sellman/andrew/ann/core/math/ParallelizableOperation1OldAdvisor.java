...
class ParallelizableOperation1OldAdvisor {
	private static final int DEFAULT_TEST_COUNT = 100;
	private final int testCount;
	private final Map<AdviceGuide, Boolean> parallelOpAdvice;

	public ParallelizableOperation1OldAdvisor(final int testCount) {
		this.testCount = testCount;
		parallelOpAdvice = new ConcurrentHashMap<>();
	}

	public ParallelizableOperation1OldAdvisor() {
		this(DEFAULT_TEST_COUNT);
	}

	public boolean doAsParrallelOp(final ParallelizableOperation1Old op, final int matrixARowCount, final int matrixAColumnCount, final int matrixBRowCount, final int matrixBColumnCount) {
	...
	private double getSequentialOpNanos(final ParallelizableOperation1Old op, final Matrix a, final Matrix b) {
		long start = System.nanoTime();
		op.doSequentialOp(a, b);
		return (System.nanoTime() - start);
	}

	private long getParallelOpNanos(final ParallelizableOperation1Old op, final Matrix a, final Matrix b) {
		long start = System.nanoTime();
		op.doParallelOp(a, b);
		return (System.nanoTime() - start);
	}
